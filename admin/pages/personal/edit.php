...
<div class="data-wrapper">
	<form class="form-horizontal" id="form-person">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" name="nama-person" class="form-control" id="nama-person" placeholder="nama" required="required" value="<?php _e( $person->GetNama() ) ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="address-person">Address <strong>*</strong></label>
			<div class="col-sm-4">
				<textarea required='required' name="address-person" class="form-control" id="address-person"><?php _e( $person->GetAlamat() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="telp-person">Telp</label>
			<div class="col-sm-4">
				<input type="number" name="telp-person" class="form-control" id="telp-person" placeholder="telp" value="<?php _e( $person->GetTelp() ) ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="deskripsi-person">Description</label>
			<div class="col-sm-4">
				<textarea name="deskripsi-person" class="form-control" id="deskripsi-person"><?php _e( $person->GetDeskripsi() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="info-lain-person">Other</label>
			<div class="col-sm-4">
				<textarea name="info-lain-person" class="form-control" id="info-lain-person"><?php _e( $person->GetOther() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
		...
		<?php for( $i = 0; $i < 5; $i++ ): ?>
		<?php $gbr = $person->GetGambars()[ $i ];//var_dump($gbr); ?>
		<?php
			if( !is_null( $gbr ) ) {
		...
		<div class="form-group">
			<div class="col-sm-offset-1 col-sm-1">
				<img id="gbr-view<?php _e($i); ?>" src="<?php if( !is_null($gbr ) ) _e( $gbr->GetLinkGambar() ) ?>" alt="" class="gambar-view-person" height="50">
			</div>
			<div class="col-sm-3">
				<input id="gbr-fname<?php _e($i); ?>" type="text" name="gambar-url-person[]" class="form-control gambar-url-person" readonly="readonly" value="<?php if( !is_null($gbr ) ) _e( end($arrLinkGambar) ); ?>" >
				<input id="gbr-id<?php _e($i); ?>" type="hidden" name="gambar-id-person[]" value="<?php if( !is_null( $gbr ) ) _e( $gbr->GetPostId() ) ; ?>" class="gambar-id-person">
			</div>
			<div class="col-sm-1">
			...
			<div class="col-sm-2">
				<input id="gbr-radio<?php _e($i); ?>" class='gambar-utama-person' type='radio' name='gambar-utama-person' value='<?php if( !is_null( $gbr ) ) _e( $gbr->GetGambarUtama() ) ; ?>' <?php if( !is_null( $gbr ) && $gbr->GetGambarUtama()==1 ) _e( 'checked="checked"' ) ; ?>> gambar utama
			</div>
		</div>
		<?php endfor; ?>
		<div class="form-group">
			...
			$.post(
				...
				function( response ) {
					...
					if( result.status ) {
						...
						//reset_form_person();
						location.href = "<?php echo admin_url('admin.php?page=sltg-personal&doaction=edit&person='); ?><?php _e($person->GetID()); ?>&status=success";
					}else{
