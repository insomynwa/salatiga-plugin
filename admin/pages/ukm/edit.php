...
<div class="data-wrapper">
	<form class="form-horizontal" id="form-ukm">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" name="nama-ukm" class="form-control" id="nama-ukm" placeholder="nama" required="required" value="<?php _e( $ukm->GetNama() ) ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="address-ukm">Address <strong>*</strong></label>
			<div class="col-sm-4">
				<textarea required='required' name="address-ukm" class="form-control" id="address-ukm"><?php _e( $ukm->GetAlamat() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="telp-ukm">Telp</label>
			<div class="col-sm-4">
				<input type="number" name="telp-ukm" class="form-control" id="telp-ukm" placeholder="telp" value="<?php _e( $ukm->GetTelp() ) ?>">
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="deskripsi-ukm">Description</label>
			<div class="col-sm-4">
				<textarea name="deskripsi-ukm" class="form-control" id="deskripsi-ukm"><?php _e( $ukm->GetDeskripsi() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="info-lain-ukm">Other</label>
			<div class="col-sm-4">
				<textarea name="info-lain-ukm" class="form-control" id="info-lain-ukm"><?php _e( $ukm->GetOther() ) ?></textarea>
			</div>
		</div>
		<div class="form-group">
			<label class="col-sm-2 control-label" for="founder-ukm">Founder <strong>*</strong></label>
			<div class="col-sm-4">
				<input type="text" name="founder-ukm" class="form-control" id="founder-ukm" placeholder="nama founder (Soekarno) " required="required" readonly value="<?php _e( $ukm->GetPemilik()->GetNama() ) ?>">
				<input type="hidden" name="founder-ukm-id" id="founder-ukm-id" value="<?php _e( $ukm->GetPemilik()->GetID() ) ?>">
			</div>
			<div class="col-sm-2">
		...
		<?php for( $i = 0; $i < 5; $i++ ): ?>
		<?php $gbr = $ukm->GetGambars()[ $i ];//var_dump($gbr); ?>
		<?php
			if( !is_null( $gbr ) ) {
		...
		<div class="form-group">
			<div class="col-sm-offset-1 col-sm-1">
				<img id="gbr-view<?php _e($i); ?>" src="<?php if( !is_null($gbr ) ) _e( $gbr->GetLinkGambar() ) ?>" alt="" class="gambar-view-ukm" height="50">
			</div>
			<div class="col-sm-3">
				<input id="gbr-fname<?php _e($i); ?>" type="text" name="gambar-url-ukm[]" class="form-control gambar-url-ukm" readonly="readonly" value="<?php if( !is_null($gbr ) ) _e( end($arrLinkGambar) ); ?>" >
				<input id="gbr-id<?php _e($i); ?>" type="hidden" name="gambar-id-ukm[]" value="<?php if( !is_null( $gbr ) ) _e( $gbr->GetPostId() ) ; ?>" class="gambar-id-ukm">
			</div>
			<div class="col-sm-1">
			...
			<div class="col-sm-2">
				<input id="gbr-radio<?php _e($i); ?>" class='gambar-utama-ukm' type='radio' name='gambar-utama-ukm' value='<?php if( !is_null( $gbr ) ) _e( $gbr->GetGambarUtama() ) ; ?>' <?php if( !is_null( $gbr ) && $gbr->GetGambarUtama()==1 ) _e( 'checked="checked"' ) ; ?>> gambar utama
			</div>
		</div>
		<?php endfor; ?>
		<div class="form-group">
			...
			$.post(
				...
				function( response ) {
					...
					if( result.status ) {
						...
						//reset_form_ukm();
						location.href = "<?php echo admin_url('admin.php?page=sltg-ukm&doaction=edit&ukm='); ?><?php _e($ukm->GetID()); ?>&status=success";
					}else{
